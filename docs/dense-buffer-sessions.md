...
- Versioning mirrors dense attributes: encoded descriptors expose `version` (and index views add `topologyVersion`). Cache the last seen version to decide when to re-upload; dirty flags remain for compatibility but log deprecation warnings.
- Call `updateDenseColorEncoded*Attribute(name)` to repack; bind via `getDenseColorEncoded*AttributeView(name)` inside `withBufferAccess`.
